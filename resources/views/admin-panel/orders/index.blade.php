                    ...
                    <table class="table table-bordered table-hover w-100 m-0">
                        ...
                        @foreach($orders as $order)
                            <tr>
                                ...
                                <td><a class="row-link"
                                       href="{{route('clients.show',$order->client)}}">{{$order->client->name}}</a></td>
                                <td><a class="row-link"
                                       href="{{route('sellers.show',$order->seller)}}">{{$order->seller->restaurant_name}}</a></td>
                                <td>{{$order->notes}}</td>
                                <td>{{$order->delivery_address}}</td>
                                <td>{{$order->order_status}}</td>
                                <td><a class="row-link"
                                       href="{{route('payment-methods.show',$order->paymentMethod)}}">{{$order->paymentMethod->name}}</a></td>
